...
SRC_DIR_LIST=(${CMAKE_SRC_DIR} ${NASM_SRC_DIR} ${ASTYLE_SRC_DIR} ${OPENSSL_SRC_DIR} ${CURL_SRC_DIR} ${GFLAGS_SRC_DIR} ${GTEST_SRC_DIR} ${LIBUNWIND_SRC_DIR} ${GPERFTOOLS_SRC_DIR} ${LIBUUID_SRC_DIR} ${LIBEVENT_SRC_DIR} ${LIBXML2_SRC_DIR} ${LOG4CPLUS_SRC_DIR} ${HTTP_PARSER_SRC_DIR} ${PROTOBUF_SRC_DIR} ${RAPIDJSON_SRC_DIR} ${JSONCPP_SRC_DIR} ${MARIADB_CONNECTOR_SRC_DIR} ${HIREDIS_SRC_DIR} ${REDIS_SRC_DIR} ${ZOOKEEPER_SRC_DIR})

LIB_LIST=(cmake nasm astyle openssl curl gflags gtest libunwind gperftools libuuid libevent libxml2 log4cplus http_parser protobuf rapidjson jsoncpp mariadb_connector hiredis redis zookeeper)
